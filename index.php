...
foreach($definedLanguages as $lang => $language) $langs[] = '<a href=language?'.$lang.' title="'.L($language).'">'.$language.'</a>';

sendPageToClient(L('What are you gonna do awesome today?'),
                '<nav>'
                .(isset($client)?'<a href=start-challenge class=btn>'.L('Start a challenge').'</a> ':'')
                .'<a href=about class=btn>'.L('About').'</a> '
                .'<a href=faq class=btn title="'.L('Frequently Asked Questions').'">FAQ</a></nav>'
                .'<h1>'.L('What are you gonna do awesome today?').'</h1>'
                .$html.L('In other languages').' : '.implode(', ', $langs));
?>